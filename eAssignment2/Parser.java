public class Parser {
    ...
    TK f_statement[] = {TK.ID, TK.SWAP, TK.PRINT, TK.PRINTNL, TK.IF, TK.DO, TK.FA, TK.none};
    ...
    private void declarations() {
        ...
        if(symtab.add_var_entry(id, lno)) {
              gcprint("int");
              gcprintid(id);
              gcprint("= -12345;");
        }
    ...
    private void swapproc() {
        gcprint("void swap(int * id1, int * id2) {");
    ...
    private void swap() {
    	...
    	referenced_id(id1, true, lno1);
    	referenced_id(id1, false, lno1);
    	gcprint("&x_" + id1 + ", ");
    	...
    	referenced_id(id2, true, lno2);
            referenced_id(id2, false, lno2);
            gcprint("&x_" + id2 + ");");

    	if(id1.equals(id2)) {				// Check if 2 variables are the same
    		System.err.println("swapping variable " + id1 + " with itself on line " + lno1);
    	}
    } 
	...
	private void elseproc(){
		mustbe(TK.ELSE);
		guarded_commands(TK.ELSE);
		//mustbe(TK.ESLE);
	}
